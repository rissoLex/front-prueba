...
export class ModificarRolComponent implements OnInit {
  ...
  onChangeRolSelected(lang:any){
    ...
    this.formGRol = new FormGroup({
      nombreRol: new FormControl(rolSelec.nombreRol),
      descRol: new FormControl(rolSelec.descripcionRol),
    });
    this.endLoading();
  }
  ...
  guardarRol(){
    ...
    this.restService.consumeApiPost(jsonRequest, dataConst.CAPACIDAD_MODIFICAR_ROL)
                  .pipe( catchError(err => { 
                                      this.SpinnerService.hide();
                                      this.mostrarModalError();
                                      return throwError(() => new Error(err));
