...
export class EliminarUserComponent implements OnInit {
  ...
  constructor(private restService: RestServiceService,
    ...
    createFormUser(){
      this.formGUser = new FormGroup({
        rolsel: new FormControl({value:'', disabled:true}),
        nombreUser: new FormControl({value:'', disabled:true}),
    ...
    eliminarUser(){
      ...
      this.restService.consumeApiPost(jsonRequest, dataConst.CAPACIDAD_ELIMINAR_USER)
                    .pipe( catchError(err => { 
                                        this.SpinnerService.hide();
                                        this.mostrarModalError();
                                        return throwError(() => new Error(err));
